...
async function main() {
  ...
  for (const product of filteredProducts) {
    ...
    if (!repId) continue;
    const rep = productMap.get(repId);
    if (!rep) continue;
    for (const intent of INTENTS) {
      const fromSlug = makeSlug(product, intent);
      const toSlug = makeSlug(rep, intent);
      if (fromSlug && toSlug && fromSlug !== toSlug) {
        articleRedirects[fromSlug] = toSlug;
      }
    }
  }
  const redirectsPath = path.join(root, 'lib/article-redirects.js');
  await fs.mkdir(path.dirname(redirectsPath), { recursive: true });
  await fs.writeFile(
    redirectsPath,
    `// Auto-generated by scripts/generate-article-pages.js — do not edit by hand.\n// Maps old (deduplicated) article slugs to their canonical representative slugs.\nexport const articleRedirects = ${JSON.stringify(articleRedirects, null, 2)};\n`,
    'utf8'
  );
  ...
  console.log(`Wrote ${Object.keys(articleRedirects).length} article redirects to lib/article-redirects.js`);
